...
export const icons: {
  ...
  (icon: React.FunctionComponent<IconProps>) => styled(icon).withConfig({
    ...
    & .icon-fill {
        fill: ${p => (p.color && Colors[p.color] ? Colors[p.color] : 'blue')};
      }
  
      & .icon-stroke {
        stroke: ${p => (p.color && Colors[p.color] ? Colors[p.color] : 'blue')};
      }
    `
);
